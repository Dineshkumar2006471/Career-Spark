...
function Roadmap() {
  ...
  async function downloadRoadmap() {
    ...
    doc.setFontSize(13)
    doc.text('90-Day Learning Path', margin, 37)
    ...
    doc.save(`CareerSpark_LearningPath_${targetRole.replace(/\s+/g, '_')}.pdf`)
  }
    ...
    <div className="space-y-xl">
        ...
        <div className="relative flex flex-wrap items-start justify-between gap-lg">
          <div>
            <div className="inline-flex items-center gap-sm rounded-full border border-primary/20 bg-primary/5 px-4 py-1.5 text-xs font-bold uppercase tracking-wider text-primary mb-4">
              <Map size={14} /> 90-Day Learning Path
            </div>
            <h2 className="font-display text-3xl font-bold">{targetRole}</h2>
            <p className="mt-sm max-w-xl text-sm leading-relaxed text-body">Your personalized learning path with weekly actions, courses, certifications, and internship links.</p>
            {/* Progress Bar */}
          ...
          <div className="flex gap-sm">
            ...
            >
              ...
              {isRegenerating ? 'Generating...' : 'Regenerate AI Learning Path'}
            </button>
      ...
      {phases.length === 0 ? (
        <div className="rounded-3xl border-2 border-dashed border-primary/20 p-12 text-center bg-canvas">
          ...
          <h3 className="text-xl font-bold font-display text-ink mb-2">No Learning Path Generated</h3>
          <p className="text-body text-sm max-w-md mx-auto mb-6">
            Your personalized 90-day learning path hasn't been generated yet. Click the button above to build it using AI.
          </p>
        </div>
      ) : (
